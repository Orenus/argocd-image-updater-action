...
describe('testing login cases', () => {
    ...
    test('login fail with network problem should throw', async () => {
        ...
        // TODO: make work...
        expect(async () => await argo.login('kuku', 'muku', serverHost)).toThrow('Network Error')
    })
});
...
describe('testing applications api', () => {
    ...
    test('get apps returns many apps happy path', async () => {
        ...
        mock.onGet(`${apiURL}/applications`, { project: 'dummy' }).reply(200, { items: [mockResultItem, mockResultItem, mockResultItem] })
        const context: ArgoCDContext = new ArgoCDContext();
        context.project = 'dummy';
        const apps: GitOpsAppInfo[] = await argo.getApps(context);
    ...
    test('update image helm existing param happy path', async () => {
        ...
        const mockAppReply = [{
            spec: {
                source: {
                    helm: {
                        parameters: [
                            { name: 'name', value: 'some.image:1.0.3' }
                        ]
                    }
                }
            }
        ...
        mock
            .onGet(`${apiURL}/applications/${appName}/manifests`).reply(200, mockSourceTypeReply)
            .onGet(`${apiURL}/applications`, { name: appName }).reply(200, mockAppReply)
            .onPatch(`${apiURL}/applications/${appName}`).reply(200, payload);
